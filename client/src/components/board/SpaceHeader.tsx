...
export default function SpaceHeader({ name, onAddMember, canAddMember }: Props) {
  return (
    <div className="px-4 pt-4 pb-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2"><button className="p-1.5 border border-[#45464a] text-[#b7b9bc]"><Share2 size={15} /></button>{canAddMember && <button onClick={onAddMember} className="text-[#b7b9bc] border border-[#45464a] px-2.5 py-1.5 text-xs hover:bg-[#292a2d]"><Users size={13} className="inline mr-1.5" />Add member</button>}</div>
      </div>
    </div>
  );
}
